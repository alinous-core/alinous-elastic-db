...
{
	throw (new(ctx) BTreeException(ConstStr::getCNST_STR_1627(), ctx));
}
...
void MBTreeMaxNode::printNode(StringBuffer* buff, int level, ThreadContext* ctx)
{
	...
	buff->append(ConstStr::getCNST_STR_1628(), ctx)->append(ConstStr::getCNST_STR_1629(), ctx)->append(ConstStr::getCNST_STR_1625(), ctx);
	...
	node->printNode(buff, nextLevel, ctx);
	printTab(buff, level, ctx);
	buff->append(ConstStr::getCNST_STR_1630(), ctx);
}
