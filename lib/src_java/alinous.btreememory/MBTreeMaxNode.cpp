...
{
	throw (new(ctx) BTreeException(ConstStr::getCNST_STR_1562(), ctx));
}
...
void MBTreeMaxNode::printNode(StringBuffer* buff, int level, ThreadContext* ctx)
{
	...
	buff->append(ConstStr::getCNST_STR_1563(), ctx)->append(ConstStr::getCNST_STR_1564(), ctx)->append(ConstStr::getCNST_STR_1560(), ctx);
...
{
	throw (new(ctx) BTreeException(ConstStr::getCNST_STR_1566(), ctx));
}
