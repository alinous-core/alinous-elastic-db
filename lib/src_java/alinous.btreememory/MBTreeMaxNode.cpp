...
void MBTreeMaxNode::printNode(StringBuffer* buff, int level, ThreadContext* ctx)
{
	...
	buff->append(ConstStr::getCNST_STR_1635(), ctx)->append(ConstStr::getCNST_STR_1636(), ctx)->append(ConstStr::getCNST_STR_1632(), ctx);
	...
	node->printNode(buff, nextLevel, ctx);
	printTab(buff, level, ctx);
	buff->append(ConstStr::getCNST_STR_1637(), ctx);
}
