...
{
	throw (new(ctx) BTreeException(ConstStr::getCNST_STR_1547(), ctx));
}
...
void MBTreeMaxNode::printNode(StringBuffer* buff, int level, ThreadContext* ctx)
{
	...
	buff->append(ConstStr::getCNST_STR_1548(), ctx)->append(ConstStr::getCNST_STR_1549(), ctx)->append(ConstStr::getCNST_STR_1545(), ctx);
	...
	node->printNode(buff, nextLevel, ctx);
	printTab(buff, level, ctx);
	buff->append(ConstStr::getCNST_STR_1550(), ctx);
}
...
{
	throw (new(ctx) BTreeException(ConstStr::getCNST_STR_1551(), ctx));
}
