...
{
	throw (new(ctx) BTreeException(ConstStr::getCNST_STR_1635(), ctx));
}
...
void MBTreeMaxNode::printNode(StringBuffer* buff, int level, ThreadContext* ctx)
{
	...
	buff->append(ConstStr::getCNST_STR_1636(), ctx)->append(ConstStr::getCNST_STR_1637(), ctx)->append(ConstStr::getCNST_STR_1633(), ctx);
	...
	node->printNode(buff, nextLevel, ctx);
	printTab(buff, level, ctx);
	buff->append(ConstStr::getCNST_STR_1638(), ctx);
}
...
{
	throw (new(ctx) BTreeException(ConstStr::getCNST_STR_1639(), ctx));
}
