...
void MBTreeNode::printNode(StringBuffer* buff, int level, ThreadContext* ctx)
{
	...
	buff->append(ConstStr::getCNST_STR_1648(), ctx)->append(getKey(ctx)->toString(ctx), ctx)->append(ConstStr::getCNST_STR_1634(), ctx);
	...
	printTab(buff, level, ctx);
	buff->append(ConstStr::getCNST_STR_1649(), ctx);
}
