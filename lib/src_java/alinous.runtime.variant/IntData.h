#ifndef ALINOUS_RUNTIME_VARIANT_INTDATA_H_
#define ALINOUS_RUNTIME_VARIANT_INTDATA_H_
...
class IntData final : public IVariantData, public virtual IObject {
public:
	IntData(const IntData& base) = default;
public:
	IntData(int data, ThreadContext* ctx) throw() ;
	void __construct_impl(int data, ThreadContext* ctx) throw() ;
	IntData(ThreadContext* ctx) throw()  : IObject(ctx), IVariantData(ctx), data(0)
	{
	}
	void __construct_impl(ThreadContext* ctx) throw() 
	{
	}
	virtual ~IntData() throw();
...
public:
	static IntData* importFromXml(DomNode* node, ThreadContext* ctx) throw() ;
public:
...
#endif /* end of ALINOUS_RUNTIME_VARIANT_INTDATA_H_ */
