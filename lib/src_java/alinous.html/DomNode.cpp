...
{
	if(this->name->equals(ConstStr::getCNST_STR_1756(), ctx) && singleEnd)
	{
		throw (new(ctx) RuntimeException(ConstStr::getCNST_STR_1757(), ctx));
	}
...
String* DomNode::toString(ThreadContext* ctx) throw() 
{
		...
		else 
	{
		...
		buff->append(ConstStr::getCNST_STR_1759(), ctx);
		buff->append(this->name, ctx);
		buff->append(ConstStr::getCNST_STR_82(), ctx);
	}
	return buff->toString(ctx);
