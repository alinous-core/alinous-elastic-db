...
{
	if(this->name->equals(ConstStr::getCNST_STR_1742(), ctx) && singleEnd)
	{
		throw (new(ctx) RuntimeException(ConstStr::getCNST_STR_1743(), ctx));
	}
...
String* DomNode::toString(ThreadContext* ctx) throw() 
{
		...
		else 
	{
		...
		buff->append(ConstStr::getCNST_STR_1745(), ctx);
		buff->append(this->name, ctx);
		buff->append(ConstStr::getCNST_STR_82(), ctx);
	}
	return buff->toString(ctx);
