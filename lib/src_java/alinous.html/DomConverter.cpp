...
bool DomConverter::__init_static_variables(){
	...
	ThreadContext* ctx = ThreadContext::newThreadContext();
	{
		...
		__GC_MV(nullptr, &(QUOTE), ConstStr::getCNST_STR_1748(), String);
		__GC_MV(nullptr, &(DOUBLE_QUOTE), ConstStr::getCNST_STR_1310(), String);
		__GC_MV(nullptr, &(SRC), ConstStr::getCNST_STR_1749(), String);
		__GC_MV(nullptr, &(EQUALS), ConstStr::getCNST_STR_1076(), String);
		__GC_MV(nullptr, &(A), ConstStr::getCNST_STR_1750(), String);
		__GC_MV(nullptr, &(IMG), ConstStr::getCNST_STR_1751(), String);
		...
		__GC_MV(nullptr, &(BACK_SLASH), ConstStr::getCNST_STR_1752(), String);
	}
	ctx->localGC();
	delete ctx;
	return true;
...
{
	if(token->endsWith(ConstStr::getCNST_STR_1745(), ctx))
	{
...
void DomConverter::handleIntag(String* token, ThreadContext* ctx) throw() 
{
	...
	if(!this->tagSet)
	{
		...
		if(token->startsWith(ConstStr::getCNST_STR_1746(), ctx))
		{
...
void DomConverter::handleJavaScriptCode(String* token, ThreadContext* ctx) throw() 
{
	...
	if(token->equals(DomConverter::LT, ctx))
	{
		...
		if(tagEnds->equals(ConstStr::getCNST_STR_1007(), ctx) && tagName->toLowerCase(ctx)->equals(ConstStr::getCNST_STR_1747(), ctx))
		{
			this->replacer->addBodyString(this->buffer->toString(ctx), ctx);
			changeStatus(Status::intag, ctx);
			this->tagSet = false;
...
bool DomConverter::isJavaScript(ThreadContext* ctx) throw() 
{
	...
	if(this->replacer->getCurrentTag(ctx)->getName(ctx)->toLowerCase(ctx)->equals(ConstStr::getCNST_STR_1747(), ctx))
	{
		return true;
	}
	return false;
