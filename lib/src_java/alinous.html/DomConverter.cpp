...
bool DomConverter::__init_static_variables(){
	...
	ThreadContext* ctx = ThreadContext::newThreadContext();
	{
		...
		__GC_MV(nullptr, &(QUOTE), ConstStr::getCNST_STR_1635(), String);
		__GC_MV(nullptr, &(DOUBLE_QUOTE), ConstStr::getCNST_STR_1207(), String);
		__GC_MV(nullptr, &(SRC), ConstStr::getCNST_STR_1636(), String);
		__GC_MV(nullptr, &(EQUALS), ConstStr::getCNST_STR_1030(), String);
		__GC_MV(nullptr, &(A), ConstStr::getCNST_STR_1637(), String);
		__GC_MV(nullptr, &(IMG), ConstStr::getCNST_STR_1638(), String);
		...
		__GC_MV(nullptr, &(BACK_SLASH), ConstStr::getCNST_STR_1639(), String);
	}
	ctx->localGC();
	delete ctx;
	return true;
...
{
	if(token->endsWith(ConstStr::getCNST_STR_1632(), ctx))
	{
...
void DomConverter::handleIntag(String* token, ThreadContext* ctx) throw() 
{
	...
	if(!this->tagSet)
	{
		...
		if(token->startsWith(ConstStr::getCNST_STR_1633(), ctx))
		{
...
void DomConverter::handleJavaScriptCode(String* token, ThreadContext* ctx) throw() 
{
	...
	if(token->equals(DomConverter::LT, ctx))
	{
		...
		if(tagEnds->equals(ConstStr::getCNST_STR_984(), ctx) && tagName->toLowerCase(ctx)->equals(ConstStr::getCNST_STR_1634(), ctx))
		{
			this->replacer->addBodyString(this->buffer->toString(ctx), ctx);
			changeStatus(Status::intag, ctx);
			this->tagSet = false;
...
bool DomConverter::isJavaScript(ThreadContext* ctx) throw() 
{
	...
	if(this->replacer->getCurrentTag(ctx)->getName(ctx)->toLowerCase(ctx)->equals(ConstStr::getCNST_STR_1634(), ctx))
	{
		return true;
	}
	return false;
