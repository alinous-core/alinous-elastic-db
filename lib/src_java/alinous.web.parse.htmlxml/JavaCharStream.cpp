...
wchar_t JavaCharStream::readChar(ThreadContext* ctx)
{
	...
	if((buffer->get(bufpos)) == L'\\')
	{
		...
		}
		{
			...
			{
				throw (new(ctx) Error(ConstStr::getCNST_STR_1259()->clone(ctx)->append(line, ctx)->append(ConstStr::getCNST_STR_1260(), ctx)->append(column, ctx)->append(ConstStr::getCNST_STR_947(), ctx), ctx));
			}
		}
		if(backSlashCnt == 1)
		{
