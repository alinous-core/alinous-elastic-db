...
bool Xpath2Compare::getBooleanValue(DomDocument* document, DomNode* currentNode, ThreadContext* ctx)
{
	...
	if((dynamic_cast<NumberValue*>(leftValue) != 0) && (dynamic_cast<NumberValue*>(rightValue) != 0))
	{
		...
		if(this->ope->equals(ConstStr::getCNST_STR_1140(), ctx))
		{
			return l == r;
		}
				else 
		{
			if(this->ope->equals(ConstStr::getCNST_STR_1806(), ctx))
			{
				return l != r;
			}
						else 
			{
				...
				{
					if(this->ope->equals(ConstStr::getCNST_STR_1058(), ctx))
					{
						return l >= r;
					}
										else 
					{
						if(this->ope->equals(ConstStr::getCNST_STR_1056(), ctx))
						{
							return l < r;
						}
												else 
						{
							if(this->ope->equals(ConstStr::getCNST_STR_1057(), ctx))
							{
								return l <= r;
							}
						}
...
bool Xpath2Compare::checkExists(DomDocument* document, DomNode* currentNode, ThreadContext* ctx)
{
	...
	{
		throw (new(ctx) MatchingException(ConstStr::getCNST_STR_1807(), ctx));
	}
