...
{
	__GC_MV(this, &(this->storage), (new(ctx) FileStorage(diskCache, file, ConstStr::getCNST_STR_1551(), ctx)), FileStorage);
...
void BTree::open(ThreadContext* ctx)
{
	...
	{
		throw (new(ctx) BTreeException(ConstStr::getCNST_STR_1552(), ctx));
	}
	this->storage->open(ctx);
