...
{
	__GC_MV(this, &(this->storage), (new(ctx) FileStorage(diskCache, file, ConstStr::getCNST_STR_1589(), ctx)), FileStorage);
...
void BTree::open(ThreadContext* ctx)
{
	...
	{
		throw (new(ctx) BTreeException(ConstStr::getCNST_STR_1590(), ctx));
	}
	this->storage->open(ctx);
