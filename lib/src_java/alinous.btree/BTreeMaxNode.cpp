...
void BTreeMaxNode::printNode(StringBuffer* buff, int level, ThreadContext* ctx)
{
	...
	buff->append(ConstStr::getCNST_STR_1643(), ctx)->append(ConstStr::getCNST_STR_1644(), ctx)->append(ConstStr::getCNST_STR_1640(), ctx);
...
{
	throw (new(ctx) BTreeException(ConstStr::getCNST_STR_1646(), ctx));
}
