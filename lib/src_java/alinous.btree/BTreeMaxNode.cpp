...
void BTreeMaxNode::printNode(StringBuffer* buff, int level, ThreadContext* ctx)
{
	...
	buff->append(ConstStr::getCNST_STR_1574(), ctx)->append(ConstStr::getCNST_STR_1575(), ctx)->append(ConstStr::getCNST_STR_1571(), ctx);
	...
	node->endUse(ctx);
	printTab(buff, level, ctx);
	buff->append(ConstStr::getCNST_STR_1576(), ctx);
}
