...
{
	throw (new(ctx) BTreeException(ConstStr::getCNST_STR_1584(), ctx));
}
...
void BTreeNode::printNode(StringBuffer* buff, int level, ThreadContext* ctx)
{
	...
	buff->append(ConstStr::getCNST_STR_1585(), ctx)->append(getKey(ctx)->toString(ctx), ctx)->append(ConstStr::getCNST_STR_1571(), ctx);
	...
	printTab(buff, level, ctx);
	buff->append(ConstStr::getCNST_STR_1586(), ctx);
}
