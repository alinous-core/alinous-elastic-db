...
{
	throw (new(ctx) BTreeException(ConstStr::getCNST_STR_1558(), ctx));
}
...
void BTreeNode::printNode(StringBuffer* buff, int level, ThreadContext* ctx)
{
	...
	buff->append(ConstStr::getCNST_STR_1559(), ctx)->append(getKey(ctx)->toString(ctx), ctx)->append(ConstStr::getCNST_STR_1545(), ctx);
	...
	printTab(buff, level, ctx);
	buff->append(ConstStr::getCNST_STR_1560(), ctx);
}
