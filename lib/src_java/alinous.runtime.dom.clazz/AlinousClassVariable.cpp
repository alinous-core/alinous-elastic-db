...
String* AlinousClassVariable::CLASS_VARIABLE = ConstStr::getCNST_STR_1063();
String* AlinousClassVariable::CLASS_TYPE = ConstStr::getCNST_STR_1064();
String* AlinousClassVariable::MEMBERS = ConstStr::getCNST_STR_1063();
...
void AlinousClassVariable::outDebugXml(DomNode* parentNode, String* nm, ThreadContext* ctx) throw() 
{
	...
	buff->append(name->getPackageName(ctx), ctx)->append(ConstStr::getCNST_STR_947(), ctx)->append(name->getClassName(ctx), ctx);
...
{
	throw (new(ctx) VariableException(ConstStr::getCNST_STR_1062(), ctx));
}
...
String* AlinousClassVariable::toString(ThreadContext* ctx) throw() 
{
	...
	while(it->hasNext(ctx))
	{
		...
		buff->append(key, ctx)->append(ConstStr::getCNST_STR_1030(), ctx);
		if(val != nullptr)
		{
