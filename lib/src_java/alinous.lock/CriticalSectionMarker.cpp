...
{
	{
		...
		{
			throw (new(ctx) CriticalSectionMarkerException(ConstStr::getCNST_STR_1667(), ctx));
		}
		this->critical = true;
	}
}
...
{
	{
		...
		{
			throw (new(ctx) CriticalSectionMarkerException(ConstStr::getCNST_STR_1668(), ctx));
		}
		this->critical = false;
	}
}
...
{
	{
		...
		if(this->critical == true)
		{
			throw (new(ctx) CriticalSectionMarkerException(ConstStr::getCNST_STR_1669(), ctx));
		}
	}
}
}}
