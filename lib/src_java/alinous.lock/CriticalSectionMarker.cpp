...
{
	{
		...
		{
			throw (new(ctx) CriticalSectionMarkerException(ConstStr::getCNST_STR_1630(), ctx));
		}
		this->critical = true;
	}
}
...
{
	{
		...
		{
			throw (new(ctx) CriticalSectionMarkerException(ConstStr::getCNST_STR_1631(), ctx));
		}
		this->critical = false;
	}
}
...
{
	{
		...
		if(this->critical == true)
		{
			throw (new(ctx) CriticalSectionMarkerException(ConstStr::getCNST_STR_1632(), ctx));
		}
	}
}
}}
