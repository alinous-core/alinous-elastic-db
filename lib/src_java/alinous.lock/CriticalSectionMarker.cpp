...
{
	{
		...
		{
			throw (new(ctx) CriticalSectionMarkerException(ConstStr::getCNST_STR_1732(), ctx));
		}
		this->critical = true;
	}
}
...
{
	{
		...
		{
			throw (new(ctx) CriticalSectionMarkerException(ConstStr::getCNST_STR_1733(), ctx));
		}
		this->critical = false;
	}
}
...
{
	{
		...
		if(this->critical == true)
		{
			throw (new(ctx) CriticalSectionMarkerException(ConstStr::getCNST_STR_1734(), ctx));
		}
	}
}
}}
