...
void HttpHeaderProcessor::process(ThreadContext* ctx)
{
	...
	if(this->method->equals(ConstStr::getCNST_STR_3510(), ctx))
	{
		parseGetParams(ctx);
	}
	String* contentLength = this->requestHeaders->get(ConstStr::getCNST_STR_1854(), ctx);
	...
	int length = 0;
	{
		...
		{
			throw (new(ctx) AlinousException(ConstStr::getCNST_STR_3511(), e, ctx));
		}
	}
	String* contentType = this->requestHeaders->get(ConstStr::getCNST_STR_3503(), ctx);
	if(contentType->startsWith(ConstStr::getCNST_STR_3512(), ctx))
	{
		HttpParamHandler::postMimeParams(this->stream, length, contentType, ctx);
	}
		else 
	{
		GCUtils<HashMap<String,AbstractHttpParameter> >::mv(this, &(this->params), HttpParamHandler::postParams(this->stream, length, ConstStr::getCNST_STR_1096(), ctx), ctx);
	}
}
...
void HttpHeaderProcessor::parseGetParams(ThreadContext* ctx)
{
	...
	__GC_MV(this, &(this->path), this->path->substring(0, index, ctx), String);
	GCUtils<HashMap<String,AbstractHttpParameter> >::mv(this, &(this->params), PostUrlEncodedReader::parseString(paramPart, ConstStr::getCNST_STR_1096(), ctx), ctx);
}
void HttpHeaderProcessor::parseFirstLine(String* line, ThreadContext* ctx)
{
	...
	if(pos < 0)
	{
		throw (new(ctx) AlinousException(ConstStr::getCNST_STR_3513(), ctx));
	}
	int beginIndex = 0;
	...
	if(pos < 0)
	{
		throw (new(ctx) AlinousException(ConstStr::getCNST_STR_3514(), ctx));
	}
	...
	if(pos <= beginIndex)
	{
		throw (new(ctx) AlinousException(ConstStr::getCNST_STR_3515(), ctx));
	}
