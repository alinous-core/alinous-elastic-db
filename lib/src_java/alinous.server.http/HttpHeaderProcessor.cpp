...
void HttpHeaderProcessor::process(ThreadContext* ctx)
{
	...
	if(this->method->equals(ConstStr::getCNST_STR_3515(), ctx))
	{
		parseGetParams(ctx);
	}
	String* contentLength = this->requestHeaders->get(ConstStr::getCNST_STR_1859(), ctx);
	...
	int length = 0;
	{
		...
		{
			throw (new(ctx) AlinousException(ConstStr::getCNST_STR_3516(), e, ctx));
		}
	}
	String* contentType = this->requestHeaders->get(ConstStr::getCNST_STR_3508(), ctx);
	if(contentType->startsWith(ConstStr::getCNST_STR_3517(), ctx))
	{
...
void HttpHeaderProcessor::parseFirstLine(String* line, ThreadContext* ctx)
{
	...
	if(pos <= beginIndex)
	{
		throw (new(ctx) AlinousException(ConstStr::getCNST_STR_3520(), ctx));
	}
...
void HttpHeaderProcessor::parseLine(String* line, ThreadContext* ctx)
{
	...
	if(splitter < 0)
	{
		throw (new(ctx) AlinousException(ConstStr::getCNST_STR_3521(), ctx));
	}
