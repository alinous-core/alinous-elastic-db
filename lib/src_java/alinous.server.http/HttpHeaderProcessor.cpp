...
void HttpHeaderProcessor::process(ThreadContext* ctx)
{
	...
	if(this->method->equals(ConstStr::getCNST_STR_3459(), ctx))
	{
		parseGetParams(ctx);
	}
	String* contentLength = this->requestHeaders->get(ConstStr::getCNST_STR_1803(), ctx);
	...
	String* contentType = this->requestHeaders->get(ConstStr::getCNST_STR_3452(), ctx);
	if(contentType->startsWith(ConstStr::getCNST_STR_3461(), ctx))
	{
...
void HttpHeaderProcessor::parseLine(String* line, ThreadContext* ctx)
{
	...
	if(splitter < 0)
	{
		throw (new(ctx) AlinousException(ConstStr::getCNST_STR_3465(), ctx));
	}
