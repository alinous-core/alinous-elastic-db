...
void HttpHeaderProcessor::process(ThreadContext* ctx)
{
	...
	if(this->method->equals(ConstStr::getCNST_STR_3544(), ctx))
	{
		parseGetParams(ctx);
	}
	String* contentLength = this->requestHeaders->get(ConstStr::getCNST_STR_1888(), ctx);
	...
	int length = 0;
	{
		...
		{
			throw (new(ctx) AlinousException(ConstStr::getCNST_STR_3545(), e, ctx));
		}
	}
	String* contentType = this->requestHeaders->get(ConstStr::getCNST_STR_3537(), ctx);
	if(contentType->startsWith(ConstStr::getCNST_STR_3546(), ctx))
	{
