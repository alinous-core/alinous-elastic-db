...
void HttpHeaderProcessor::process(ThreadContext* ctx)
{
	...
	if(this->method->equals(ConstStr::getCNST_STR_3458(), ctx))
	{
		parseGetParams(ctx);
	}
	String* contentLength = this->requestHeaders->get(ConstStr::getCNST_STR_1802(), ctx);
	...
	int length = 0;
	{
		...
		{
			throw (new(ctx) AlinousException(ConstStr::getCNST_STR_3459(), e, ctx));
		}
	}
	String* contentType = this->requestHeaders->get(ConstStr::getCNST_STR_3451(), ctx);
	if(contentType->startsWith(ConstStr::getCNST_STR_3460(), ctx))
	{
