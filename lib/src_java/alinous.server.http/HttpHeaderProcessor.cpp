...
void HttpHeaderProcessor::process(ThreadContext* ctx)
{
	...
	if(this->method->equals(ConstStr::getCNST_STR_3524(), ctx))
	{
		parseGetParams(ctx);
	}
	String* contentLength = this->requestHeaders->get(ConstStr::getCNST_STR_1868(), ctx);
	...
	int length = 0;
	{
		...
		{
			throw (new(ctx) AlinousException(ConstStr::getCNST_STR_3525(), e, ctx));
		}
	}
	String* contentType = this->requestHeaders->get(ConstStr::getCNST_STR_3517(), ctx);
	if(contentType->startsWith(ConstStr::getCNST_STR_3526(), ctx))
	{
		HttpParamHandler::postMimeParams(this->stream, length, contentType, ctx);
	}
		else 
	{
		GCUtils<HashMap<String,AbstractHttpParameter> >::mv(this, &(this->params), HttpParamHandler::postParams(this->stream, length, ConstStr::getCNST_STR_1101(), ctx), ctx);
	}
}
...
void HttpHeaderProcessor::parseGetParams(ThreadContext* ctx)
{
	...
	__GC_MV(this, &(this->path), this->path->substring(0, index, ctx), String);
	GCUtils<HashMap<String,AbstractHttpParameter> >::mv(this, &(this->params), PostUrlEncodedReader::parseString(paramPart, ConstStr::getCNST_STR_1101(), ctx), ctx);
}
void HttpHeaderProcessor::parseFirstLine(String* line, ThreadContext* ctx)
{
	...
	if(pos < 0)
	{
		throw (new(ctx) AlinousException(ConstStr::getCNST_STR_3527(), ctx));
	}
	int beginIndex = 0;
	...
	if(pos < 0)
	{
		throw (new(ctx) AlinousException(ConstStr::getCNST_STR_3528(), ctx));
	}
	...
	if(pos <= beginIndex)
	{
		throw (new(ctx) AlinousException(ConstStr::getCNST_STR_3529(), ctx));
	}
...
void HttpHeaderProcessor::parseLine(String* line, ThreadContext* ctx)
{
	...
	if(splitter < 0)
	{
		throw (new(ctx) AlinousException(ConstStr::getCNST_STR_3530(), ctx));
	}
