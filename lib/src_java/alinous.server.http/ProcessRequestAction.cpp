...
String* ProcessRequestAction::endStr = ConstStr::getCNST_STR_3469();
...
void ProcessRequestAction::outResponse(int contentLength, OutputStream* outStream, ThreadContext* ctx)
{
	...
	buff->append(ConstStr::getCNST_STR_3464(), ctx);
	buff->append(ConstStr::getCNST_STR_3465(), ctx);
	if(contentLength > 0)
	{
		buff->append(ConstStr::getCNST_STR_3466(), ctx)->append(contentLength, ctx)->append(ConstStr::getCNST_STR_1794(), ctx);
	}
