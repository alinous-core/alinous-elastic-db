...
String* ProcessRequestAction::endStr = ConstStr::getCNST_STR_3467();
...
void ProcessRequestAction::outResponse(int contentLength, OutputStream* outStream, ThreadContext* ctx)
{
	...
	buff->append(ConstStr::getCNST_STR_3462(), ctx);
	buff->append(ConstStr::getCNST_STR_3463(), ctx);
	if(contentLength > 0)
	{
		buff->append(ConstStr::getCNST_STR_3464(), ctx)->append(contentLength, ctx)->append(ConstStr::getCNST_STR_1792(), ctx);
	}
	buff->append(ConstStr::getCNST_STR_3465(), ctx);
	buff->append(ConstStr::getCNST_STR_1792(), ctx);
