...
String* ProcessRequestAction::endStr = ConstStr::getCNST_STR_3429();
...
void ProcessRequestAction::outResponse(int contentLength, OutputStream* outStream, ThreadContext* ctx)
{
	...
	buff->append(ConstStr::getCNST_STR_3424(), ctx);
	buff->append(ConstStr::getCNST_STR_3425(), ctx);
	if(contentLength > 0)
	{
		buff->append(ConstStr::getCNST_STR_3426(), ctx)->append(contentLength, ctx)->append(ConstStr::getCNST_STR_1754(), ctx);
	}
	buff->append(ConstStr::getCNST_STR_3427(), ctx);
