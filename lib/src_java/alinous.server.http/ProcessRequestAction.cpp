...
String* ProcessRequestAction::endStr = ConstStr::getCNST_STR_3529();
...
void ProcessRequestAction::outResponse(int contentLength, OutputStream* outStream, ThreadContext* ctx)
{
	...
	if(contentLength > 0)
	{
		buff->append(ConstStr::getCNST_STR_3526(), ctx)->append(contentLength, ctx)->append(ConstStr::getCNST_STR_1854(), ctx);
	}
	buff->append(ConstStr::getCNST_STR_3527(), ctx);
	buff->append(ConstStr::getCNST_STR_1854(), ctx);
