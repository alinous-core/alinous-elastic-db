...
String* ProcessRequestAction::endStr = ConstStr::getCNST_STR_3554();
...
void ProcessRequestAction::outResponse(int contentLength, OutputStream* outStream, ThreadContext* ctx)
{
	...
	buff->append(ConstStr::getCNST_STR_3549(), ctx);
	buff->append(ConstStr::getCNST_STR_3550(), ctx);
	if(contentLength > 0)
	{
		buff->append(ConstStr::getCNST_STR_3551(), ctx)->append(contentLength, ctx)->append(ConstStr::getCNST_STR_1879(), ctx);
	}
	buff->append(ConstStr::getCNST_STR_3552(), ctx);
	buff->append(ConstStr::getCNST_STR_1879(), ctx);
...
{
	StringBuffer* buff = (new(ctx) StringBuffer(ctx));
	buff->append(ConstStr::getCNST_STR_3553(), ctx);
	buff->append(ConstStr::getCNST_STR_3550(), ctx);
	buff->append(ConstStr::getCNST_STR_3552(), ctx);
	buff->append(ConstStr::getCNST_STR_1879(), ctx);
	if(!result->head)
