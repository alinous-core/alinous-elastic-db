...
String* ProcessRequestAction::endStr = ConstStr::getCNST_STR_3468();
...
void ProcessRequestAction::outResponse(int contentLength, OutputStream* outStream, ThreadContext* ctx)
{
	...
	if(contentLength > 0)
	{
		buff->append(ConstStr::getCNST_STR_3465(), ctx)->append(contentLength, ctx)->append(ConstStr::getCNST_STR_1793(), ctx);
	}
	buff->append(ConstStr::getCNST_STR_3466(), ctx);
	buff->append(ConstStr::getCNST_STR_1793(), ctx);
