...
String* ProcessRequestAction::endStr = ConstStr::getCNST_STR_3538();
...
void ProcessRequestAction::outResponse(int contentLength, OutputStream* outStream, ThreadContext* ctx)
{
	...
	if(contentLength > 0)
	{
		buff->append(ConstStr::getCNST_STR_3535(), ctx)->append(contentLength, ctx)->append(ConstStr::getCNST_STR_1863(), ctx);
	}
	buff->append(ConstStr::getCNST_STR_3536(), ctx);
	buff->append(ConstStr::getCNST_STR_1863(), ctx);
...
{
	StringBuffer* buff = (new(ctx) StringBuffer(ctx));
	buff->append(ConstStr::getCNST_STR_3537(), ctx);
	buff->append(ConstStr::getCNST_STR_3534(), ctx);
	buff->append(ConstStr::getCNST_STR_3536(), ctx);
	buff->append(ConstStr::getCNST_STR_1863(), ctx);
	if(!result->head)
