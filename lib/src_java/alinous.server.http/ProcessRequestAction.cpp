...
String* ProcessRequestAction::endStr = ConstStr::getCNST_STR_3463();
...
void ProcessRequestAction::outResponse(int contentLength, OutputStream* outStream, ThreadContext* ctx)
{
	...
	buff->append(ConstStr::getCNST_STR_3459(), ctx);
	if(contentLength > 0)
	{
		buff->append(ConstStr::getCNST_STR_3460(), ctx)->append(contentLength, ctx)->append(ConstStr::getCNST_STR_1788(), ctx);
	}
	buff->append(ConstStr::getCNST_STR_3461(), ctx);
	buff->append(ConstStr::getCNST_STR_1788(), ctx);
