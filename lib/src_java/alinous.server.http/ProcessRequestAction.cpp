...
String* ProcessRequestAction::endStr = ConstStr::getCNST_STR_3555();
...
void ProcessRequestAction::outResponse(int contentLength, OutputStream* outStream, ThreadContext* ctx)
{
	...
	buff->append(ConstStr::getCNST_STR_3551(), ctx);
	if(contentLength > 0)
	{
		buff->append(ConstStr::getCNST_STR_3552(), ctx)->append(contentLength, ctx)->append(ConstStr::getCNST_STR_1880(), ctx);
	}
	buff->append(ConstStr::getCNST_STR_3553(), ctx);
	buff->append(ConstStr::getCNST_STR_1880(), ctx);
