...
void MimeFormParameterDecoder::decode(InputStream* inStream, String* boundary, int contentLength, ThreadContext* ctx)
{
	...
	__GC_MV(this, &(this->boundary), ConstStr::getCNST_STR_1745()->clone(ctx)->append(boundary, ctx), String);
	__GC_MV(this, &(this->boundaryEnd), boundary->clone(ctx)->append(ConstStr::getCNST_STR_1745(), ctx), String);
	this->readBytes = 0;
	String* line = readHeaderLine(ctx);
	if(!line->equals(this->boundary, ctx))
	{
		throw (new(ctx) IOException(ConstStr::getCNST_STR_3523(), ctx));
	}
	do
	{
		handleMimePart(ctx);
