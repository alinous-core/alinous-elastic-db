...
String* MimeHeader::CONTENT_DESCRIPTION = ConstStr::getCNST_STR_3438();
String* MimeHeader::CONTENT_TYPE = ConstStr::getCNST_STR_3439();
...
MimeHeader* MimeHeader::init(String* line, ThreadContext* ctx)
{
	...
	if(pos < 0)
	{
		throw (new(ctx) IOException(ConstStr::getCNST_STR_3437(), ctx));
	}
...
{
	if(value->startsWith(ConstStr::getCNST_STR_1237(), ctx) && value->endsWith(ConstStr::getCNST_STR_1237(), ctx) && value->length(ctx) >= 2)
	{
