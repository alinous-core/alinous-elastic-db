...
String* MimeHeader::CONTENT_DESCRIPTION = ConstStr::getCNST_STR_3535();
String* MimeHeader::CONTENT_TYPE = ConstStr::getCNST_STR_3536();
...
MimeHeader* MimeHeader::init(String* line, ThreadContext* ctx)
{
	...
	if(pos < 0)
	{
		throw (new(ctx) IOException(ConstStr::getCNST_STR_3534(), ctx));
	}
