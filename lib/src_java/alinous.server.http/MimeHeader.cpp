...
String* MimeHeader::CONTENT_DESCRIPTION = ConstStr::getCNST_STR_3505();
String* MimeHeader::CONTENT_TYPE = ConstStr::getCNST_STR_3506();
...
{
	if(value->startsWith(ConstStr::getCNST_STR_1301(), ctx) && value->endsWith(ConstStr::getCNST_STR_1301(), ctx) && value->length(ctx) >= 2)
	{
...
{
	int idx = value->indexOf(ConstStr::getCNST_STR_1072(), ctx);
	if(idx < 0)
	{
		throw (new(ctx) IOException(ConstStr::getCNST_STR_3504(), ctx));
	}
