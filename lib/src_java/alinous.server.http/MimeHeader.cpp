...
String* MimeHeader::CONTENT_DESCRIPTION = ConstStr::getCNST_STR_3512();
String* MimeHeader::CONTENT_TYPE = ConstStr::getCNST_STR_3513();
...
MimeHeader* MimeHeader::init(String* line, ThreadContext* ctx)
{
	...
	if(pos < 0)
	{
		throw (new(ctx) IOException(ConstStr::getCNST_STR_3511(), ctx));
	}
