...
String* MimeHeader::CONTENT_DESCRIPTION = ConstStr::getCNST_STR_3516();
String* MimeHeader::CONTENT_TYPE = ConstStr::getCNST_STR_3517();
...
MimeHeader* MimeHeader::init(String* line, ThreadContext* ctx)
{
	...
	if(pos < 0)
	{
		throw (new(ctx) IOException(ConstStr::getCNST_STR_3515(), ctx));
	}
