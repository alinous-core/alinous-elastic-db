...
String* MimeHeader::CONTENT_DESCRIPTION = ConstStr::getCNST_STR_3507();
String* MimeHeader::CONTENT_TYPE = ConstStr::getCNST_STR_3508();
...
MimeHeader* MimeHeader::init(String* line, ThreadContext* ctx)
{
	...
	if(pos < 0)
	{
		throw (new(ctx) IOException(ConstStr::getCNST_STR_3506(), ctx));
	}
...
{
	if(value->startsWith(ConstStr::getCNST_STR_1302(), ctx) && value->endsWith(ConstStr::getCNST_STR_1302(), ctx) && value->length(ctx) >= 2)
	{
...
{
	int idx = value->indexOf(ConstStr::getCNST_STR_1073(), ctx);
	if(idx < 0)
	{
		throw (new(ctx) IOException(ConstStr::getCNST_STR_3506(), ctx));
	}
