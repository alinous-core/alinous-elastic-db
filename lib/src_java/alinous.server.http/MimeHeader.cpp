...
String* MimeHeader::CONTENT_DESCRIPTION = ConstStr::getCNST_STR_3502();
String* MimeHeader::CONTENT_TYPE = ConstStr::getCNST_STR_3503();
...
MimeHeader* MimeHeader::init(String* line, ThreadContext* ctx)
{
	...
	if(pos < 0)
	{
		throw (new(ctx) IOException(ConstStr::getCNST_STR_3501(), ctx));
	}
...
{
	if(value->startsWith(ConstStr::getCNST_STR_1298(), ctx) && value->endsWith(ConstStr::getCNST_STR_1298(), ctx) && value->length(ctx) >= 2)
	{
...
{
	int idx = value->indexOf(ConstStr::getCNST_STR_1071(), ctx);
	if(idx < 0)
	{
		throw (new(ctx) IOException(ConstStr::getCNST_STR_3501(), ctx));
	}
