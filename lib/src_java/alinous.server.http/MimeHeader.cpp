...
String* MimeHeader::CONTENT_DESCRIPTION = ConstStr::getCNST_STR_3517();
String* MimeHeader::CONTENT_TYPE = ConstStr::getCNST_STR_3518();
...
MimeHeader* MimeHeader::init(String* line, ThreadContext* ctx)
{
	...
	if(pos < 0)
	{
		throw (new(ctx) IOException(ConstStr::getCNST_STR_3516(), ctx));
	}
...
{
	if(value->startsWith(ConstStr::getCNST_STR_1308(), ctx) && value->endsWith(ConstStr::getCNST_STR_1308(), ctx) && value->length(ctx) >= 2)
	{
