...
String* MimeHeader::CONTENT_DESCRIPTION = ConstStr::getCNST_STR_3441();
String* MimeHeader::CONTENT_TYPE = ConstStr::getCNST_STR_3442();
...
MimeHeader* MimeHeader::init(String* line, ThreadContext* ctx)
{
	...
	if(pos < 0)
	{
		throw (new(ctx) IOException(ConstStr::getCNST_STR_3440(), ctx));
	}
...
{
	if(value->startsWith(ConstStr::getCNST_STR_1240(), ctx) && value->endsWith(ConstStr::getCNST_STR_1240(), ctx) && value->length(ctx) >= 2)
	{
