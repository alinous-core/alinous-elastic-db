...
ScanResultRecord* ScanResult::getRecord(long long soid, ThreadContext* ctx)
{
	...
	{
		throw (new(ctx) BTreeException(ConstStr::getCNST_STR_1728(), ctx));
	}
...
String* ScanResult::getOidIndexName(ThreadContext* ctx) throw() 
{
	if(this->storagePath == nullptr)
	{
		...
		buff->append(ConstStr::getCNST_STR_1726(), ctx)->append(this->serial, ctx)->append(ConstStr::getCNST_STR_1727(), ctx);
