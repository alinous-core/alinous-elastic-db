...
{
	{
		...
		{
			throw (new(ctx) AlinousException(ConstStr::getCNST_STR_1662(), e, ctx));
		}
...
ScanResultRecord* ScanResult::getRecord(long long soid, ThreadContext* ctx)
{
	...
	{
		throw (new(ctx) BTreeException(ConstStr::getCNST_STR_1665(), ctx));
	}
...
String* ScanResult::getOidIndexName(ThreadContext* ctx) throw() 
{
	if(this->storagePath == nullptr)
	{
		...
		buff->append(ConstStr::getCNST_STR_1663(), ctx)->append(this->serial, ctx)->append(ConstStr::getCNST_STR_1664(), ctx);
