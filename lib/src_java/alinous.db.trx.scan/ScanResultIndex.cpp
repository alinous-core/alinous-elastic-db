...
{
	{
		...
		{
			throw (new(ctx) AlinousException(ConstStr::getCNST_STR_1660(), e, ctx));
		}
...
String* ScanResultIndex::getOidIndexName(ThreadContext* ctx) throw() 
{
	if(this->storagePath == nullptr)
	{
		...
		buff->append(ConstStr::getCNST_STR_1661(), ctx)->append(this->indexSerial, ctx)->append(ConstStr::getCNST_STR_1664(), ctx);
