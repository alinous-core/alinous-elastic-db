...
bool NodeInfo::__init_static_variables(){
 ...
 NodeInfo::NodeInfo(ThreadContext* ctx) throw()  : IObject(ctx), ICommandData(ctx), host(nullptr), port(0), ipv6(0)
{
}
void NodeInfo::__construct_impl(ThreadContext* ctx) throw() 
{
}
...
NodeInfo* NodeInfo::parseUrl(String* url, bool ipv6, ThreadContext* ctx)
{
	...
	if(segs->length != 2)
	{
		throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_3580(), ctx));
	}
...
void NodeInfo::__cleanUp(ThreadContext* ctx){
}
}}}
