...
String* AlinousWebContentProcessor::PING_TO_SERVER = ConstStr::getCNST_STR_1860();
String* AlinousWebContentProcessor::PING_RETURN = ConstStr::getCNST_STR_1860();
String* AlinousWebContentProcessor::DEBUG_CMD_ACCESS = ConstStr::getCNST_STR_1861();
String* AlinousWebContentProcessor::HEAD_METHOD = ConstStr::getCNST_STR_1862();
...
ContentResult* AlinousWebContentProcessor::proccess(HttpHeaderProcessor* request, ThreadContext* ctx)
{
	...
	if(path->equals(AlinousWebContentProcessor::PING_TO_SERVER, ctx))
	{
		...
		IArrayObjectPrimitive<char>* ret = nullptr;
		{
			try
			{
				ret = pingReturn->getBytes(ConstStr::getCNST_STR_1096(), ctx);
			}
		...
		else 
	{
		...
		{
			return (new(ctx) ContentResult(DebugProcessor::process(this->core, request, ctx), ConstStr::getCNST_STR_1859(), this->mimeResolver, head, ctx));
		}
	}
