...
String* AlinousWebContentProcessor::PING_TO_SERVER = ConstStr::getCNST_STR_1792();
String* AlinousWebContentProcessor::PING_RETURN = ConstStr::getCNST_STR_1792();
String* AlinousWebContentProcessor::DEBUG_CMD_ACCESS = ConstStr::getCNST_STR_1793();
String* AlinousWebContentProcessor::HEAD_METHOD = ConstStr::getCNST_STR_1794();
...
ContentResult* AlinousWebContentProcessor::proccess(HttpHeaderProcessor* request, ThreadContext* ctx)
{
		...
		else 
	{
		...
		{
			return (new(ctx) ContentResult(DebugProcessor::process(this->core, request, ctx), ConstStr::getCNST_STR_1791(), this->mimeResolver, head, ctx));
		}
	}
