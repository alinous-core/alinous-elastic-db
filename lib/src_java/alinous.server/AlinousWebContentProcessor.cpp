...
String* AlinousWebContentProcessor::PING_TO_SERVER = ConstStr::getCNST_STR_1805();
String* AlinousWebContentProcessor::PING_RETURN = ConstStr::getCNST_STR_1805();
String* AlinousWebContentProcessor::DEBUG_CMD_ACCESS = ConstStr::getCNST_STR_1806();
String* AlinousWebContentProcessor::HEAD_METHOD = ConstStr::getCNST_STR_1807();
...
ContentResult* AlinousWebContentProcessor::proccess(HttpHeaderProcessor* request, ThreadContext* ctx)
{
		...
		else 
	{
		...
		{
			return (new(ctx) ContentResult(DebugProcessor::process(this->core, request, ctx), ConstStr::getCNST_STR_1804(), this->mimeResolver, head, ctx));
		}
	}
