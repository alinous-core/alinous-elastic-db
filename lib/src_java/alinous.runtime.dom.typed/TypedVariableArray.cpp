...
String* TypedVariableArray::VAL_TYPE = ConstStr::getCNST_STR_1204();
...
{
	throw (new(ctx) VariableException(ConstStr::getCNST_STR_1202(), ctx));
}
...
void TypedVariableArray::readData(NetworkBinaryBuffer* buff, ThreadContext* ctx)
{
	...
	for(int i = 0; i != maxLoop; ++i)
	{
		...
		{
			throw (new(ctx) VariableException(ConstStr::getCNST_STR_1203(), ctx));
		}
