...
String* StringVariable::VAL_TYPE = ConstStr::getCNST_STR_1150();
...
BoolVariable* StringVariable::toBoolVariable(ThreadContext* ctx)
{
	...
	{
		if(this->value->equalsIgnoreCase(ConstStr::getCNST_STR_1132(), ctx))
		{
			return (new(ctx) BoolVariable(false, ctx));
		}
	}
	throw (new(ctx) VariableException(ConstStr::getCNST_STR_1148(), ctx));
}
...
{
	{
		...
		{
			throw (new(ctx) VariableException(ConstStr::getCNST_STR_1149(), e, ctx));
		}
	}
}
