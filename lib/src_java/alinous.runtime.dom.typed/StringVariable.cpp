...
String* StringVariable::VAL_TYPE = ConstStr::getCNST_STR_1155();
...
BoolVariable* StringVariable::toBoolVariable(ThreadContext* ctx)
{
	...
	{
		if(this->value->equalsIgnoreCase(ConstStr::getCNST_STR_1137(), ctx))
		{
			return (new(ctx) BoolVariable(false, ctx));
		}
	}
	throw (new(ctx) VariableException(ConstStr::getCNST_STR_1153(), ctx));
}
...
{
	{
		...
		{
			throw (new(ctx) VariableException(ConstStr::getCNST_STR_1154(), e, ctx));
		}
	}
}
