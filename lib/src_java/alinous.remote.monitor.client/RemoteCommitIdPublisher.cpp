...
RemoteCommitIdPublisher* RemoteCommitIdPublisher::init(ThreadContext* ctx)
{
	...
	if(segs->length != 2)
	{
		throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_3547(), ctx));
	}
	...
	int port = 0;
	{
		...
		{
			throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_3548(), e, ctx));
		}
	}
...
long long RemoteCommitIdPublisher::getMaxCommitId(ThreadContext* ctx)
{
	GetMaxCommitIdCommand* cmd = (new(ctx) GetMaxCommitIdCommand(ctx));
	{
		...
		{
			throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_3549(), e, ctx));
		}
		catch(IOException* e)
		{
			throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_3549(), e, ctx));
