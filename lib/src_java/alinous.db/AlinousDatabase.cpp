...
void AlinousDatabase::construct(AlinousCore* core, String* dataDir, String* trxTmpDir, int maxConnection, ThreadContext* ctx)
{
	...
	__GC_MV(this, &(this->workerThreadsPool), (new(ctx) ThreadPool(16, ConstStr::getCNST_STR_1657(), ctx)), ThreadPool);
		...
		{
			{
				...
				{
					throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_1665(), e2, ctx));
				}
				catch(InterruptedException* e2)
				{
					throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_1665(), e2, ctx));
