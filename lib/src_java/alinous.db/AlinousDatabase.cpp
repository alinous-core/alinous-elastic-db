...
void AlinousDatabase::construct(AlinousCore* core, String* dataDir, String* trxTmpDir, int maxConnection, ThreadContext* ctx)
{
	...
	File* file = getConfigFile(ctx);
	{
		...
		{
			throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_1566(), e, ctx));
		}
...
void AlinousDatabase::initInstance(ThreadContext* ctx)
{
	...
	schemas->createSchema(ConstStr::getCNST_STR_951(), ctx);
	{
		...
		catch(IOException* e)
		{
			...
			throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_1567(), e, ctx));
		}
		catch(InterruptedException* e)
		{
			throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_1567(), e, ctx));
...
void AlinousDatabase::syncScheme(ThreadContext* ctx)
{
	...
	BTreeLeafNode* lvTrxIdNode = nullptr;
	{
		...
		catch(Throwable* e)
		{
			...
			e->printStackTrace(ctx);
			throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_1568(), e, ctx));
		}
	}
	{
		try
...
{
	__GC_MV(this, &(this->workerThreadsPool), (new(ctx) ThreadPool(16, ConstStr::getCNST_STR_1570(), ctx)), ThreadPool);
	...
	__GC_MV(this, &(this->trxWriterThread), (new(ctx) AlinousThread(trxLaunchJoin, ctx)), AlinousThread);
	{
		...
		catch(InterruptedException* e)
		{
			...
			__GC_MV(this, &(this->workerThreadsPool), nullptr, ThreadPool);
			throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_1571(), e, ctx));
		}
	}
		...
		{
			{
				...
				{
					throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_1572(), e2, ctx));
				}
				catch(InterruptedException* e2)
				{
					throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_1572(), e2, ctx));
			...
			throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_1572(), e, ctx));
		}
	}
	{
		try
...
File* AlinousDatabase::getConfigFile(ThreadContext* ctx) throw() 
{
	...
	{
		String* pathname = this->dataDir->clone(ctx)->append(ConstStr::getCNST_STR_1569(), ctx);
