...
void AlinousDatabase::construct(AlinousCore* core, String* dataDir, String* trxTmpDir, int maxConnection, ThreadContext* ctx)
{
	...
	__GC_MV(this, &(this->workerThreadsPool), (new(ctx) ThreadPool(16, ConstStr::getCNST_STR_1655(), ctx)), ThreadPool);
		...
		{
			{
				...
				{
					throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_1663(), e2, ctx));
				}
				catch(InterruptedException* e2)
				{
					throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_1663(), e2, ctx));
...
File* AlinousDatabase::getConfigFile(ThreadContext* ctx) throw() 
{
	...
	{
		String* pathname = this->dataDir->clone(ctx)->append(ConstStr::getCNST_STR_1661(), ctx);
