...
void AlinousDatabase::construct(AlinousCore* core, String* dataDir, String* trxTmpDir, int maxConnection, ThreadContext* ctx)
{
	...
	__GC_MV(this, &(this->workerThreadsPool), (new(ctx) ThreadPool(16, ConstStr::getCNST_STR_1595(), ctx)), ThreadPool);
...
void AlinousDatabase::syncScheme(ThreadContext* ctx)
{
	...
	BTreeLeafNode* lvTrxIdNode = nullptr;
	{
		...
		catch(Throwable* e)
		{
			...
			e->printStackTrace(ctx);
			throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_1600(), e, ctx));
		}
	}
	{
		try
...
void AlinousDatabase::open(AlinousDbInstanceInfo* instanceConfig, ThreadContext* ctx)
{
	...
	__GC_MV(this, &(this->trxWriterThread), (new(ctx) AlinousThread(trxLaunchJoin, ctx)), AlinousThread);
	{
		...
		catch(InterruptedException* e)
		{
			...
			__GC_MV(this, &(this->workerThreadsPool), nullptr, ThreadPool);
			throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_1602(), e, ctx));
		}
	}
		...
		{
			{
				...
				{
					throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_1603(), e2, ctx));
				}
				catch(InterruptedException* e2)
				{
					throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_1603(), e2, ctx));
			...
			throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_1603(), e, ctx));
		}
	}
	{
		try
	...
	}
	{
		...
		{
			throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_1604(), e, ctx));
		}
		catch(IOException* e)
		{
			throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_1604(), e, ctx));
...
File* AlinousDatabase::getConfigFile(ThreadContext* ctx) throw() 
{
	...
	{
		String* pathname = this->dataDir->clone(ctx)->append(ConstStr::getCNST_STR_1601(), ctx);
