...
String* RemoteTableStorageServer::THREAD_NAME = ConstStr::getCNST_STR_3477();
...
void RemoteTableStorageServer::init(AlinousCore* core, ThreadContext* ctx)
{
	...
	__GC_MV(this, &(this->workerThreadsPool), (new(ctx) ThreadPool(16, ConstStr::getCNST_STR_3476(), ctx)), ThreadPool);
	if(exists(ctx))
	{
		return;
	}
