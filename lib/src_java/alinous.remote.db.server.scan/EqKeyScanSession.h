#ifndef ALINOUS_REMOTE_DB_SERVER_SCAN_EQKEYSCANSESSION_H_
#define ALINOUS_REMOTE_DB_SERVER_SCAN_EQKEYSCANSESSION_H_
...
class EqKeyScanSession final : public AbstractStorageScanSession {
public:
	EqKeyScanSession(const EqKeyScanSession& base) = default;
public:
	EqKeyScanSession(IDatabaseTable* table, ClientScanCommandData* data, ThreadContext* ctx) throw() ;
	void __construct_impl(IDatabaseTable* table, ClientScanCommandData* data, ThreadContext* ctx) throw() ;
	virtual ~EqKeyScanSession() throw();
...
#endif /* end of ALINOUS_REMOTE_DB_SERVER_SCAN_EQKEYSCANSESSION_H_ */
