#ifndef ALINOUS_REMOTE_DB_SERVER_SCAN_LISTSCANSESSION_H_
#define ALINOUS_REMOTE_DB_SERVER_SCAN_LISTSCANSESSION_H_
...
class ListScanSession final : public AbstractStorageScanSession {
public:
	ListScanSession(const ListScanSession& base) = default;
public:
	ListScanSession(IDatabaseTable* table, ClientScanCommandData* data, ThreadContext* ctx) throw() ;
	void __construct_impl(IDatabaseTable* table, ClientScanCommandData* data, ThreadContext* ctx) throw() ;
	virtual ~ListScanSession() throw();
...
#endif /* end of ALINOUS_REMOTE_DB_SERVER_SCAN_LISTSCANSESSION_H_ */
