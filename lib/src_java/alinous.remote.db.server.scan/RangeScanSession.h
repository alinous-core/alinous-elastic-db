#ifndef ALINOUS_REMOTE_DB_SERVER_SCAN_RANGESCANSESSION_H_
#define ALINOUS_REMOTE_DB_SERVER_SCAN_RANGESCANSESSION_H_
...
class RangeScanSession final : public AbstractStorageScanSession {
public:
	RangeScanSession(const RangeScanSession& base) = default;
public:
	RangeScanSession(IDatabaseTable* table, ClientScanCommandData* data, ThreadContext* ctx) throw() ;
	void __construct_impl(IDatabaseTable* table, ClientScanCommandData* data, ThreadContext* ctx) throw() ;
	virtual ~RangeScanSession() throw();
...
#endif /* end of ALINOUS_REMOTE_DB_SERVER_SCAN_RANGESCANSESSION_H_ */
