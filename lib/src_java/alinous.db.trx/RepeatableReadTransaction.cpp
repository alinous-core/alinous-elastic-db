...
bool RepeatableReadTransaction::__init_static_variables(){
 ...
 RepeatableReadTransaction::RepeatableReadTransaction(DbTransactionManager* mgr, String* tmpDir, AlinousDatabase* database, ISystemLog* logger, long long trxId, ThreadContext* ctx) throw()  : IObject(ctx), DbTransaction(mgr, tmpDir, database, logger, trxId, ctx)
{
}
void RepeatableReadTransaction::__construct_impl(DbTransactionManager* mgr, String* tmpDir, AlinousDatabase* database, ISystemLog* logger, long long trxId, ThreadContext* ctx) throw() 
{
}
...
{
	{
		...
		{
			throw (new(ctx) DatabaseException(ConstStr::getCNST_STR_1588(), e, ctx));
		}
