...
int TableMetadata::fileSize(ThreadContext* ctx)
{
	...
	for(int i = 0; i != maxLoop; ++i)
	{
		TableColumnMetadata* col = this->uniqueList->get(i, ctx);
		total += col->fileSize(ctx);
	}
	total += 1;
