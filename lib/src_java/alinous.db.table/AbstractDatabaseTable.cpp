...
void AbstractDatabaseTable::open(AlinousDatabase* database, ThreadContext* ctx)
{
	getDataStorageName(ctx);
	getOidIndexName(ctx);
	__GC_MV(this, &(this->dataStorage), (new(ctx) FileStorage(database->getCore(ctx)->diskCache, (new(ctx) File(dataStoragePath, ctx)), ConstStr::getCNST_STR_1576(), ctx)), FileStorage);
	{
		try
		{
		...
		{
			throw (new(ctx) DatabaseException(ConstStr::getCNST_STR_1592(), e, ctx));
		}
	}
...
String* AbstractDatabaseTable::getDataStorageName(ThreadContext* ctx) throw() 
{
	if(this->dataStoragePath == nullptr)
	{
		...
		buff->append(this->name, ctx)->append(ConstStr::getCNST_STR_1600(), ctx);
...
void AbstractDatabaseTable::loadScheme(ThreadContext* ctx)
{
	FileStorageEntryReader* reader = 0;
	{
		...
		{
			throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_1601(), e, ctx));
		}
	}
...
String* AbstractDatabaseTable::getOidIndexName(ThreadContext* ctx) throw() 
{
	if(this->oidIndexPath == nullptr)
	{
		...
		buff->append(ConstStr::getCNST_STR_1593(), ctx)->append(this->name, ctx)->append(ConstStr::getCNST_STR_1602(), ctx);
