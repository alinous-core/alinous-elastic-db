...
void AbstractDatabaseTable::open(AlinousCore* core, BTreeGlobalCache* cache, ThreadContext* ctx)
{
	getDataStorageName(ctx);
	getOidIndexName(ctx);
	__GC_MV(this, &(this->dataStorage), (new(ctx) FileStorage(core->diskCache, (new(ctx) File(dataStoragePath, ctx)), ConstStr::getCNST_STR_1587(), ctx)), FileStorage);
	{
		try
		{
		...
		{
			throw (new(ctx) DatabaseException(ConstStr::getCNST_STR_1604(), e, ctx));
		}
	}
...
void AbstractDatabaseTable::open(bool loadscheme, AlinousCore* core, BTreeGlobalCache* cache, ThreadContext* ctx)
{
	...
	}
	{
		...
		{
			throw (new(ctx) DatabaseException(ConstStr::getCNST_STR_1611(), e, ctx));
		}
		catch(BTreeException* e)
		{
			throw (new(ctx) DatabaseException(ConstStr::getCNST_STR_1611(), e, ctx));
...
String* AbstractDatabaseTable::getDataStorageName(ThreadContext* ctx) throw() 
{
	if(this->dataStoragePath == nullptr)
	{
		...
		buff->append(this->name, ctx)->append(ConstStr::getCNST_STR_1612(), ctx);
...
String* AbstractDatabaseTable::getOidIndexName(ThreadContext* ctx) throw() 
{
	if(this->oidIndexPath == nullptr)
	{
		...
		buff->append(ConstStr::getCNST_STR_1605(), ctx)->append(this->name, ctx)->append(ConstStr::getCNST_STR_1614(), ctx);
