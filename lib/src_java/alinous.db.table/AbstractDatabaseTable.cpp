...
void AbstractDatabaseTable::open(AlinousCore* core, BTreeGlobalCache* cache, ThreadContext* ctx)
{
	getDataStorageName(ctx);
	getOidIndexName(ctx);
	__GC_MV(this, &(this->dataStorage), (new(ctx) FileStorage(core->diskCache, (new(ctx) File(dataStoragePath, ctx)), ConstStr::getCNST_STR_1656(), ctx)), FileStorage);
	{
		try
		{
		...
		{
			throw (new(ctx) DatabaseException(ConstStr::getCNST_STR_1677(), e, ctx));
		}
	}
...
void AbstractDatabaseTable::open(bool loadscheme, AlinousCore* core, BTreeGlobalCache* cache, ThreadContext* ctx)
{
	...
	}
	{
		...
		{
			throw (new(ctx) DatabaseException(ConstStr::getCNST_STR_1684(), e, ctx));
		}
		catch(BTreeException* e)
		{
			throw (new(ctx) DatabaseException(ConstStr::getCNST_STR_1684(), e, ctx));
...
String* AbstractDatabaseTable::getDataStorageName(ThreadContext* ctx) throw() 
{
	if(this->dataStoragePath == nullptr)
	{
		...
		buff->append(this->name, ctx)->append(ConstStr::getCNST_STR_1685(), ctx);
...
String* AbstractDatabaseTable::getOidIndexName(ThreadContext* ctx) throw() 
{
	if(this->oidIndexPath == nullptr)
	{
		...
		buff->append(ConstStr::getCNST_STR_1678(), ctx)->append(this->name, ctx)->append(ConstStr::getCNST_STR_1687(), ctx);
