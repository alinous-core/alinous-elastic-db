...
String* DomArray::VAL_TYPE = ConstStr::getCNST_STR_1186();
...
{
	throw (new(ctx) VariableException(ConstStr::getCNST_STR_1184(), ctx));
}
...
void DomArray::readData(NetworkBinaryBuffer* buff, ThreadContext* ctx)
{
	...
	for(int i = 0; i != maxLoop; ++i)
	{
		...
		{
			throw (new(ctx) VariableException(ConstStr::getCNST_STR_1185(), ctx));
		}
...
void DomArray::__cleanUp(ThreadContext* ctx){
}
