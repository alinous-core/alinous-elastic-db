...
String* DomArray::VAL_TYPE = ConstStr::getCNST_STR_1110();
...
{
	throw (new(ctx) VariableException(ConstStr::getCNST_STR_1108(), ctx));
}
...
void DomArray::readData(NetworkBinaryBuffer* buff, ThreadContext* ctx)
{
	...
	for(int i = 0; i != maxLoop; ++i)
	{
		...
		{
			throw (new(ctx) VariableException(ConstStr::getCNST_STR_1109(), ctx));
		}
