...
{
	TableAndSchema* tas = meta->getTas(ctx);
	String* schemaName = tas->getSchema(ctx);
	String* tableName = tas->getTable(ctx);
	...
	{
		this->logger->logWarning(ConstStr::getCNST_STR_1745()->clone(ctx)->append(meta->getindexName(ctx), ctx)->append(ConstStr::getCNST_STR_1746(), ctx), ctx);
		return;
	}
	{
		try
...
void TrxSchemeManager::executeCreateSchemaAndTable(TableSchema* sc, AlinousCore* core, ThreadContext* ctx)
{
	...
	LockObject* lockObj = this->database->instanceConfigLock;
	{
		SynchronizedBlockObj __synchronized_2(lockObj, ctx);
...
void TrxSchemeManager::__cleanUp(ThreadContext* ctx){
}
}}}}
