...
TrxRecordsCache* TrxRecordsCache::init(String* tmpDir, String* schema, String* tableName, TableMetadata* metadata, DbTransaction* trx, bool insert, ThreadContext* ctx)
{
	...
	for(int i = 0; i != maxLoop; ++i)
	{
		...
		ArrayList<String>* idxCols = imeta->getColumns(ctx);
		{
			...
			{
				throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_1640(), ctx));
			}
...
void TrxRecordsCache::switchToDisk(AlinousDatabase* database, ThreadContext* ctx)
{
	...
	File* file = (new(ctx) File(this->storagePath, ctx));
	{
		...
		{
			throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_1641(), ctx));
		}
		catch(InterruptedException* e)
		{
			throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_1641(), ctx));
...
CachedRecord* TrxRecordsCache::getRecordByOid(long long oid, ThreadContext* ctx)
{
	...
	IBTreeNode* node = nullptr;
	{
		...
		{
			throw (new(ctx) AlinousException(ConstStr::getCNST_STR_1648(), e, ctx));
		}
		catch(IOException* e)
		{
			throw (new(ctx) AlinousException(ConstStr::getCNST_STR_1649(), e, ctx));
		}
		catch(InterruptedException* e)
		{
			throw (new(ctx) AlinousException(ConstStr::getCNST_STR_1650(), e, ctx));
		}
		catch(BTreeException* e)
		{
			throw (new(ctx) AlinousException(ConstStr::getCNST_STR_1651(), e, ctx));
		}
	}
	if(node == nullptr)
	{
...
void TrxRecordsCache::insertUpdateRecord(ScanResultRecord* srecord, ThreadContext* ctx)
{
	...
	LongKey* key = (new(ctx) LongKey(srecord->getOid(ctx), ctx));
	{
		...
		{
			throw (new(ctx) AlinousException(ConstStr::getCNST_STR_1652(), e, ctx));
		}
		catch(IOException* e)
		{
			throw (new(ctx) AlinousException(ConstStr::getCNST_STR_1653(), e, ctx));
		}
		catch(BTreeException* e)
		{
			throw (new(ctx) AlinousException(ConstStr::getCNST_STR_1654(), e, ctx));
		}
	}
	for(int i = 0; i != maxIndexes; ++i)
	{
		TrxRecordCacheIndex* cacheIndex = this->indexList->get(i, ctx);
		{
			...
			{
				throw (new(ctx) AlinousException(ConstStr::getCNST_STR_1655(), e, ctx));
			}
			catch(IOException* e)
			{
				throw (new(ctx) AlinousException(ConstStr::getCNST_STR_1656(), e, ctx));
			}
			catch(BTreeException* e)
			{
				throw (new(ctx) AlinousException(ConstStr::getCNST_STR_1657(), e, ctx));
			}
		}
	}
}
...
void TrxRecordsCache::setStorageFilePath(ThreadContext* ctx) throw() 
{
	...
	String* postfix = ConstStr::getCNST_STR_1644();
	if(!this->insert)
	{
		postfix = ConstStr::getCNST_STR_1645();
	}
	buff->append(this->tmpDir, ctx)->append(this->schema, ctx)->append(ConstStr::getCNST_STR_1646(), ctx)->append(this->tableName, ctx)->append(postfix, ctx)->append(ConstStr::getCNST_STR_1647(), ctx);
