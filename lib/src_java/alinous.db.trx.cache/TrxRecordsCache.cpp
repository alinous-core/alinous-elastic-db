...
void TrxRecordsCache::setStorageFilePath(ThreadContext* ctx) throw() 
{
	...
	String* postfix = ConstStr::getCNST_STR_1701();
	if(!this->insert)
	{
		postfix = ConstStr::getCNST_STR_1702();
	}
	buff->append(this->tmpDir, ctx)->append(this->schema, ctx)->append(ConstStr::getCNST_STR_1703(), ctx)->append(this->tableName, ctx)->append(postfix, ctx)->append(ConstStr::getCNST_STR_1704(), ctx);
