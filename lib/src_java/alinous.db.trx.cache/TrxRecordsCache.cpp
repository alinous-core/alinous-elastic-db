...
TrxRecordsCache* TrxRecordsCache::init(String* tmpDir, String* schema, String* tableName, TableMetadata* metadata, DbTransaction* trx, bool insert, ThreadContext* ctx)
{
	...
	for(int i = 0; i != maxLoop; ++i)
	{
		...
		ArrayList<String>* idxCols = imeta->getColumns(ctx);
		{
			...
			{
				throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_1608(), ctx));
			}
...
CachedRecord* TrxRecordsCache::getRecordByOid(long long oid, ThreadContext* ctx)
{
	...
	IBTreeNode* node = nullptr;
	{
		...
		{
			throw (new(ctx) AlinousException(ConstStr::getCNST_STR_1616(), e, ctx));
		}
		catch(IOException* e)
		{
			throw (new(ctx) AlinousException(ConstStr::getCNST_STR_1617(), e, ctx));
		}
		catch(InterruptedException* e)
		{
			throw (new(ctx) AlinousException(ConstStr::getCNST_STR_1618(), e, ctx));
		}
		catch(BTreeException* e)
		{
			throw (new(ctx) AlinousException(ConstStr::getCNST_STR_1619(), e, ctx));
		}
	}
	if(node == nullptr)
	{
...
void TrxRecordsCache::insertUpdateRecord(ScanResultRecord* srecord, ThreadContext* ctx)
{
	...
	LongKey* key = (new(ctx) LongKey(srecord->getOid(ctx), ctx));
	{
		...
		{
			throw (new(ctx) AlinousException(ConstStr::getCNST_STR_1620(), e, ctx));
		}
		catch(IOException* e)
		{
			throw (new(ctx) AlinousException(ConstStr::getCNST_STR_1621(), e, ctx));
		}
		catch(BTreeException* e)
		{
			throw (new(ctx) AlinousException(ConstStr::getCNST_STR_1622(), e, ctx));
		}
	}
	for(int i = 0; i != maxIndexes; ++i)
	{
		TrxRecordCacheIndex* cacheIndex = this->indexList->get(i, ctx);
		{
			...
			{
				throw (new(ctx) AlinousException(ConstStr::getCNST_STR_1623(), e, ctx));
			}
			catch(IOException* e)
			{
				throw (new(ctx) AlinousException(ConstStr::getCNST_STR_1624(), e, ctx));
			}
			catch(BTreeException* e)
			{
				throw (new(ctx) AlinousException(ConstStr::getCNST_STR_1625(), e, ctx));
			}
		}
	}
}
...
void TrxRecordsCache::setStorageFilePath(ThreadContext* ctx) throw() 
{
	...
	String* postfix = ConstStr::getCNST_STR_1612();
	if(!this->insert)
	{
		postfix = ConstStr::getCNST_STR_1613();
	}
	buff->append(this->tmpDir, ctx)->append(this->schema, ctx)->append(ConstStr::getCNST_STR_1614(), ctx)->append(this->tableName, ctx)->append(postfix, ctx)->append(ConstStr::getCNST_STR_1615(), ctx);
