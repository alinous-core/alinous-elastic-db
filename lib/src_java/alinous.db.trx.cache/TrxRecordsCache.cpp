...
void TrxRecordsCache::switchToDisk(AlinousDatabase* database, ThreadContext* ctx)
{
	...
	File* file = (new(ctx) File(this->storagePath, ctx));
	{
		...
		{
			throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_1712(), ctx));
		}
		catch(InterruptedException* e)
		{
			throw (new(ctx) AlinousDbException(ConstStr::getCNST_STR_1712(), ctx));
...
void TrxRecordsCache::setStorageFilePath(ThreadContext* ctx) throw() 
{
	...
	String* postfix = ConstStr::getCNST_STR_1715();
	if(!this->insert)
	{
		postfix = ConstStr::getCNST_STR_1716();
	}
	buff->append(this->tmpDir, ctx)->append(this->schema, ctx)->append(ConstStr::getCNST_STR_1717(), ctx)->append(this->tableName, ctx)->append(postfix, ctx)->append(ConstStr::getCNST_STR_1718(), ctx);
