...
{
	{
		...
		{
			throw (new(ctx) ScanException(ConstStr::getCNST_STR_1628(), e, ctx));
		}
	}
}
bool TrxRecordCacheIndexScanner::hasNext(bool debug, ThreadContext* ctx)
{
	...
	}
	{
		...
		{
			throw (new(ctx) ScanException(ConstStr::getCNST_STR_1629(), e, ctx));
		}
		catch(InterruptedException* e)
		{
			throw (new(ctx) ScanException(ConstStr::getCNST_STR_1629(), e, ctx));
...
ScanResultRecord* TrxRecordCacheIndexScanner::toScanResultRecord(long long oid, ThreadContext* ctx)
{
	...
	{
		throw (new(ctx) ScanException(ConstStr::getCNST_STR_1630(), ctx));
	}
