...
{
	{
		...
		{
			throw (new(ctx) DatabaseException(ConstStr::getCNST_STR_1629(), e, ctx));
		}
...
{
	{
		...
		{
			throw (new(ctx) ScanException(ConstStr::getCNST_STR_1630(), e, ctx));
		}
	}
}
bool TrxRecordCacheIndexScanner::hasNext(bool debug, ThreadContext* ctx)
{
	...
	}
	{
		...
		{
			throw (new(ctx) ScanException(ConstStr::getCNST_STR_1631(), e, ctx));
		}
		catch(InterruptedException* e)
		{
			throw (new(ctx) ScanException(ConstStr::getCNST_STR_1631(), e, ctx));
...
ArrayList<ScanResultRecord>* TrxRecordCacheIndexScanner::searchByIndexValue(ScanResultIndexKey* indexValue, ThreadContext* ctx)
{
	...
	IBTreeNode* leafNode = nullptr;
	{
		...
		{
			throw (new(ctx) ScanException(ConstStr::getCNST_STR_1633(), e, ctx));
		}
		catch(IOException* e)
		{
			throw (new(ctx) ScanException(ConstStr::getCNST_STR_1633(), e, ctx));
...
ScanResultRecord* TrxRecordCacheIndexScanner::toScanResultRecord(long long oid, ThreadContext* ctx)
{
	...
	{
		throw (new(ctx) ScanException(ConstStr::getCNST_STR_1632(), ctx));
	}
