...
void HttpClient::get(String* host, int port, String* path, HttpRequestHeaders* headers, ThreadContext* ctx)
{
	...
	buff->append(ConstStr::getCNST_STR_1861(), ctx)->append(path, ctx)->append(ConstStr::getCNST_STR_1862(), ctx);
	writer->write(buff->toString(ctx), ctx);
	buff->setLength(0, ctx);
	buff->append(ConstStr::getCNST_STR_1863(), ctx)->append(host, ctx)->append(ConstStr::getCNST_STR_1859(), ctx);
	writer->write(buff->toString(ctx), ctx);
	buff->setLength(0, ctx);
	headers->writeHeader(buff, ctx);
	writer->write(buff->toString(ctx), ctx);
	writer->append(ConstStr::getCNST_STR_1859(), ctx)->flush(ctx);
	...
	String* transferEncoding = this->responseHeader->get(ConstStr::getCNST_STR_1864(), ctx);
	String* contentLength = this->responseHeader->get(ConstStr::getCNST_STR_1865(), ctx);
	if(transferEncoding != nullptr && transferEncoding->equals(ConstStr::getCNST_STR_1866(), ctx))
	{
...
{
	{
		...
		{
			throw (new(ctx) AlinousException(ConstStr::getCNST_STR_1867(), e, ctx));
		}
	}
}
...
void HttpClient::analyseHeaderLine(String* line, ThreadContext* ctx)
{
	...
	if(splitter < 0)
	{
		...
		if(codes->length < 3)
		{
			throw (new(ctx) AlinousException(ConstStr::getCNST_STR_1868(), ctx));
		}
		return;
	}
