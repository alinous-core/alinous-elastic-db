...
void HttpClient::get(String* host, int port, String* path, HttpRequestHeaders* headers, ThreadContext* ctx)
{
	...
	buff->append(ConstStr::getCNST_STR_1795(), ctx)->append(path, ctx)->append(ConstStr::getCNST_STR_1796(), ctx);
	writer->write(buff->toString(ctx), ctx);
	buff->setLength(0, ctx);
	buff->append(ConstStr::getCNST_STR_1797(), ctx)->append(host, ctx)->append(ConstStr::getCNST_STR_1793(), ctx);
	writer->write(buff->toString(ctx), ctx);
	buff->setLength(0, ctx);
	headers->writeHeader(buff, ctx);
	writer->write(buff->toString(ctx), ctx);
	writer->append(ConstStr::getCNST_STR_1793(), ctx)->flush(ctx);
	...
	String* transferEncoding = this->responseHeader->get(ConstStr::getCNST_STR_1798(), ctx);
	String* contentLength = this->responseHeader->get(ConstStr::getCNST_STR_1799(), ctx);
	if(transferEncoding != nullptr && transferEncoding->equals(ConstStr::getCNST_STR_1800(), ctx))
	{
...
{
	{
		...
		{
			throw (new(ctx) AlinousException(ConstStr::getCNST_STR_1801(), e, ctx));
		}
	}
}
...
void HttpClient::analyseHeaderLine(String* line, ThreadContext* ctx)
{
	...
	if(splitter < 0)
	{
		...
		if(codes->length < 3)
		{
			throw (new(ctx) AlinousException(ConstStr::getCNST_STR_1802(), ctx));
		}
		return;
	}
