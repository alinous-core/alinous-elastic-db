...
void HttpClient::get(String* host, int port, String* path, HttpRequestHeaders* headers, ThreadContext* ctx)
{
	...
	OutputStreamWriter* writer = (new(ctx) OutputStreamWriter(out, ConstStr::getCNST_STR_1047(), ctx));
	StringBuffer* buff = (new(ctx) StringBuffer(ctx));
	buff->append(ConstStr::getCNST_STR_1767(), ctx)->append(path, ctx)->append(ConstStr::getCNST_STR_1768(), ctx);
	writer->write(buff->toString(ctx), ctx);
	buff->setLength(0, ctx);
	buff->append(ConstStr::getCNST_STR_1769(), ctx)->append(host, ctx)->append(ConstStr::getCNST_STR_1765(), ctx);
	writer->write(buff->toString(ctx), ctx);
	buff->setLength(0, ctx);
	headers->writeHeader(buff, ctx);
	writer->write(buff->toString(ctx), ctx);
	writer->append(ConstStr::getCNST_STR_1765(), ctx)->flush(ctx);
	...
	String* transferEncoding = this->responseHeader->get(ConstStr::getCNST_STR_1770(), ctx);
	String* contentLength = this->responseHeader->get(ConstStr::getCNST_STR_1771(), ctx);
	if(transferEncoding != nullptr && transferEncoding->equals(ConstStr::getCNST_STR_1772(), ctx))
	{
...
void HttpClient::dispose(ThreadContext* ctx) throw() 
{
	...
	}
	{
		try
		{
			...
			__GC_MV(this, &(this->socket), nullptr, Socket);
		}
		catch(IOException* e)
		{
...
{
	__GC_MV(this, &(this->socket), (new(ctx) Socket(host, port, ctx)), Socket);
}
...
{
	{
		...
		{
			throw (new(ctx) AlinousException(ConstStr::getCNST_STR_1773(), e, ctx));
		}
	}
}
...
void HttpClient::analyseHeaderLine(String* line, ThreadContext* ctx)
{
	...
	if(splitter < 0)
	{
		...
		if(codes->length < 3)
		{
			throw (new(ctx) AlinousException(ConstStr::getCNST_STR_1774(), ctx));
		}
		return;
	}
