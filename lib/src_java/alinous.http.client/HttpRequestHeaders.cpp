...
{
	dowrite(buff, ConstStr::getCNST_STR_1866(), this->userAgent, ctx);
	dowrite(buff, ConstStr::getCNST_STR_1867(), this->accept, ctx);
	dowrite(buff, ConstStr::getCNST_STR_1868(), this->acceptLanguage, ctx);
	dowrite(buff, ConstStr::getCNST_STR_1869(), this->acceptEncoding, ctx);
	dowrite(buff, ConstStr::getCNST_STR_1870(), this->acceptCharset, ctx);
	dowrite(buff, ConstStr::getCNST_STR_1871(), this->keepAlive, ctx);
	dowrite(buff, ConstStr::getCNST_STR_1872(), this->connection, ctx);
	dowrite(buff, ConstStr::getCNST_STR_1873(), this->cookie, ctx);
}
...
void HttpRequestHeaders::dowrite(StringBuffer* writer, String* key, String* value, ThreadContext* ctx)
{
	...
	writer->append(key, ctx)->append(ConstStr::getCNST_STR_1874(), ctx)->append(value, ctx)->append(ConstStr::getCNST_STR_1875(), ctx);
}
}}}
