...
bool HttpRequestHeaders::__init_static_variables(){
 ...
 HttpRequestHeaders::HttpRequestHeaders(ThreadContext* ctx) throw()  : IObject(ctx), userAgent(nullptr), accept(__GC_INS(this, ConstStr::getCNST_STR_1890(), String)), acceptLanguage(nullptr), acceptEncoding(nullptr), acceptCharset(nullptr), keepAlive(nullptr), connection(nullptr), cookie(nullptr)
{
}
...
{
	dowrite(buff, ConstStr::getCNST_STR_1880(), this->userAgent, ctx);
	dowrite(buff, ConstStr::getCNST_STR_1881(), this->accept, ctx);
	dowrite(buff, ConstStr::getCNST_STR_1882(), this->acceptLanguage, ctx);
	dowrite(buff, ConstStr::getCNST_STR_1883(), this->acceptEncoding, ctx);
	dowrite(buff, ConstStr::getCNST_STR_1884(), this->acceptCharset, ctx);
	dowrite(buff, ConstStr::getCNST_STR_1885(), this->keepAlive, ctx);
	dowrite(buff, ConstStr::getCNST_STR_1886(), this->connection, ctx);
	dowrite(buff, ConstStr::getCNST_STR_1887(), this->cookie, ctx);
}
...
void HttpRequestHeaders::dowrite(StringBuffer* writer, String* key, String* value, ThreadContext* ctx)
{
	...
	writer->append(key, ctx)->append(ConstStr::getCNST_STR_1888(), ctx)->append(value, ctx)->append(ConstStr::getCNST_STR_1889(), ctx);
}
