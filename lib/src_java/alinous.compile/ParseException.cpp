...
void ParseException::__construct_impl(Token* currentTokenVal, IArrayObject<IArrayObjectPrimitive<long long>>* expectedTokenSequencesVal, IArrayObject<String>* tokenImageVal, ThreadContext* ctx) throw() 
{
 ...
 ParseException::ParseException(ThreadContext* ctx) throw()  : IObject(ctx), Exception(ctx), currentToken(nullptr), expectedTokenSequences(nullptr), tokenImage(nullptr), eol(__GC_INS(this, System::getProperty(ConstStr::getCNST_STR_1315(), ConstStr::getCNST_STR_1209(), ctx), String))
{
}
void ParseException::__construct_impl(ThreadContext* ctx) throw() 
{
}
 ParseException::ParseException(String* message, ThreadContext* ctx) throw()  : IObject(ctx), Exception(message, ctx), currentToken(nullptr), expectedTokenSequences(nullptr), tokenImage(nullptr), eol(__GC_INS(this, System::getProperty(ConstStr::getCNST_STR_1315(), ConstStr::getCNST_STR_1209(), ctx), String))
{
}
...
String* ParseException::add_escapes(String* str, ThreadContext* ctx) throw() 
{
	...
	for(long long i = 0; i < (long long)str->length(ctx); i ++ )
	{
		...
		case L'\"':
			...
			continue;
		case L'\'':
			retval->append(ConstStr::getCNST_STR_1304(), ctx);
			continue;
		case L'\\':
		...
		default:
			...
			if((ch) < (wchar_t)0x20 || ch > (wchar_t)0x7e)
			{
				...
				retval->append(ConstStr::getCNST_STR_1305()->clone(ctx)->append(s->substring(s->length(ctx) - 4, s->length(ctx), ctx), ctx), ctx);
			}
						else 
			{
...
{
	String* eol = System::getProperty(ConstStr::getCNST_STR_1315(), ConstStr::getCNST_STR_1209(), ctx);
	...
	for(long long i = 0; i < (long long)expectedTokenSequences->length; i ++ )
	{
		...
		{
			expected->append(ConstStr::getCNST_STR_1316(), ctx);
		}
		expected->append(eol, ctx)->append(ConstStr::getCNST_STR_1317(), ctx);
	}
	String* retval = ConstStr::getCNST_STR_1318();
	...
	for(long long i = 0; i < maxSize; i ++ )
	{
		...
		retval->append(ConstStr::getCNST_STR_1319(), ctx);
		retval->append(add_escapes(tok->image, ctx), ctx);
		retval->append(ConstStr::getCNST_STR_1319(), ctx);
		tok = tok->next;
	}
	retval->append(ConstStr::getCNST_STR_1320()->clone(ctx)->append(currentToken->next->beginLine, ctx)->append(ConstStr::getCNST_STR_1307(), ctx)->append(currentToken->next->beginColumn, ctx), ctx);
	retval->append(ConstStr::getCNST_STR_950()->clone(ctx)->append(eol, ctx), ctx);
	if(expectedTokenSequences->length == 1)
	{
		retval->append(ConstStr::getCNST_STR_1321()->clone(ctx)->append(eol, ctx)->append(ConstStr::getCNST_STR_1317(), ctx), ctx);
	}
		else 
	{
		retval->append(ConstStr::getCNST_STR_1322()->clone(ctx)->append(eol, ctx)->append(ConstStr::getCNST_STR_1317(), ctx), ctx);
	}
