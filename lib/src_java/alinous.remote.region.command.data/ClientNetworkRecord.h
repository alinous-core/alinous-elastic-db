#ifndef ALINOUS_REMOTE_REGION_COMMAND_DATA_CLIENTNETWORKRECORD_H_
#define ALINOUS_REMOTE_REGION_COMMAND_DATA_CLIENTNETWORKRECORD_H_
...
public:
	...
	ClientNetworkRecord(ThreadContext* ctx) throw()  : IObject(ctx), IDatabaseRecord(ctx), ICommandData(ctx), oid(0), lastUpdateCommitId(0), insertedCommitId(0), values(GCUtils<ArrayList<VariantValue> >::ins(this, (new(ctx) ArrayList<VariantValue>(ctx)), ctx, __FILEW__, __LINE__, L""))
	{
	}
...
#endif /* end of ALINOUS_REMOTE_REGION_COMMAND_DATA_CLIENTNETWORKRECORD_H_ */
