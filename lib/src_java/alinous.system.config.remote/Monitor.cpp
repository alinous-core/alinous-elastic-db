...
Monitor* Monitor::parseInstance(MatchCandidate* candidate, DomDocument* document, Matcher* matcher, ThreadContext* ctx)
{
	...
	IVariableValue* attr = selfDom->getAttributeValue(ConstStr::getCNST_STR_1258(), ctx);
	if(attr == nullptr)
	{
		throw (new(ctx) AlinousInitException(ConstStr::getCNST_STR_1278(), ctx));
	}
	ref->setPort(attr->toString(ctx)->trim(ctx), ctx);
	attr = selfDom->getAttributeValue(ConstStr::getCNST_STR_1270(), ctx);
	if(attr != nullptr)
	{
	...
	MatchCandidatesCollection* result = matcher->match(document, selfDom, ConstStr::getCNST_STR_1280(), ctx);
