...
RegionsRef* RegionsRef::parseInstance(MatchCandidate* candidate, DomDocument* document, Matcher* matcher, ThreadContext* ctx)
{
	...
	MatchCandidatesCollection* result = matcher->match(document, selfDom, ConstStr::getCNST_STR_1204(), ctx);
	...
	while(it->hasNext(ctx))
	{
		...
		if(attr == nullptr)
		{
			throw (new(ctx) AlinousInitException(ConstStr::getCNST_STR_1205(), ctx));
		}
		String* name = attr->toString(ctx)->trim(ctx);
		ref->setName(name, ctx);
		attr = dom->getAttributeValue(ConstStr::getCNST_STR_1202(), ctx);
		if(attr == nullptr)
		{
