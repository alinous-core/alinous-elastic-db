...
RegionsRef* RegionsRef::parseInstance(MatchCandidate* candidate, DomDocument* document, Matcher* matcher, ThreadContext* ctx)
{
	...
	MatchCandidatesCollection* result = matcher->match(document, selfDom, ConstStr::getCNST_STR_1229(), ctx);
	...
	while(it->hasNext(ctx))
	{
		...
		if(attr == nullptr)
		{
			throw (new(ctx) AlinousInitException(ConstStr::getCNST_STR_1231(), ctx));
		}
