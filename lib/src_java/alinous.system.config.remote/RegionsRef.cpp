...
RegionsRef* RegionsRef::parseInstance(MatchCandidate* candidate, DomDocument* document, Matcher* matcher, ThreadContext* ctx)
{
	...
	MatchCandidatesCollection* result = matcher->match(document, selfDom, ConstStr::getCNST_STR_1011(), ctx);
	...
	while(it->hasNext(ctx))
	{
		...
		if(attr == nullptr)
		{
			throw (new(ctx) AlinousInitException(ConstStr::getCNST_STR_1012(), ctx));
		}
		String* name = attr->toString(ctx)->trim(ctx);
		ref->setName(name, ctx);
		attr = dom->getAttributeValue(ConstStr::getCNST_STR_986(), ctx);
		if(attr == nullptr)
		{
			throw (new(ctx) AlinousInitException(ConstStr::getCNST_STR_1013(), ctx));
		}
